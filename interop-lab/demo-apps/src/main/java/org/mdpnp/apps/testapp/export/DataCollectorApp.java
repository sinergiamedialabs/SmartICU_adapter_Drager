    ...
    public void handleDataSampleEvent(NumericsDataCollector.NumericSampleEvent evt) throws Exception {
        ...
        Platform.runLater(new Runnable() {
            public void run() {
                ...
                List<Device> name = deviceListModel.getContents().stream().filter(s -> s.getUDI().equalsIgnoreCase(row.getUniqueDeviceIdentifier())).collect(Collectors.toList());
                JsonModal jm=new JsonModal();
                jm.setDeviceId(row.getUniqueDeviceIdentifier());
                jm.setDeviceName(name.get(0).getModel());
                jm.setMetricId(row.getMetricId());
                jm.setValue(row.getValue());

                try {
                    ObjectWriter ow = new ObjectMapper().writer().withDefaultPrettyPrinter();
                    String json = ow.writeValueAsString(jm);
